...
class CustomGraphBased(BaseGraphSelection):
	...
	def __init__(self, n_samples, metric='euclidean', 
		...
		super(CustomGraphBased, self).__init__(n_samples=n_samples, 
	...
	def fit(self, X, y=None, sample_weight=None, sample_cost=None):
		...
		return super(CustomGraphBased, self).fit(X, y=y, 
			sample_weight=sample_weight, sample_cost=sample_cost)

	def _initialize(self, X_pairwise):
		super(CustomGraphBased, self)._initialize(X_pairwise)
	...
	def _calculate_sieve_gains(self, X_pairwise, thresholds, idxs):
		...
		super(CustomGraphBased, self)._calculate_sieve_gains(
	...
	def _select_next(self, X_pairwise, gain, idx):
		...
		super(CustomGraphBased, self)._select_next(
			X_pairwise, gain, idx)
...
if __name__ == '__main__':  
    ...
    model = CustomGraphBased(999, metric='euclidean', optimizer='approximate-lazy')
    start = time.time()
    model.fit(X)
